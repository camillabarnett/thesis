...
*uk 
*individual 
capture drop camsis_m
gen camsis_m =  ISCO88 if ISCO88 >999 & sex==1
sum camsis_m

capture drop camsis_f
gen camsis_f =  ISCO88 if ISCO88 >999 & sex==2
sum camsis_f

do "H:\thesis_analysis\do_files\my_dofiles\other\ukcamsis_isco88_noemp"
sum camsis_m
sum camsis_f

capture drop  uk_camsis_m
capture drop  uk_camsis_f
rename camsis_m uk_camsis_m
rename camsis_f uk_camsis_f
*partner 

capture drop camsis_f
gen camsis_f =  SPISCO88 if SPISCO88 >999 & sex==1
sum camsis_f

capture drop camsis_m
gen camsis_m =  SPISCO88 if SPISCO88 >999 & sex==2
sum camsis_m
...
capture drop  s_uk_camsis_m
capture drop  s_uk_camsis_f
rename camsis_m s_uk_camsis_m
rename camsis_f s_uk_camsis_f

capture drop ukcamsis
gen ukcamsis=.
replace ukcamsis = uk_camsis_f if sex==2
replace ukcamsis = uk_camsis_m if sex==1

capture drop s_ukcamsis
gen s_ukcamsis=.
replace s_ukcamsis = s_uk_camsis_f if sex==1
replace s_ukcamsis = s_uk_camsis_m if sex==2
*france - not possible
...
rename camsis_m se_camsis_m
rename camsis_f se_camsis_f
*partner 
...
capture drop  s_se_camsis_m
capture drop  s_se_camsis_f
rename camsis_m s_se_camsis_m
rename camsis_f s_se_camsis_f

capture drop secamsis
gen secamsis=.
replace secamsis = se_camsis_f if sex==2
replace secamsis = se_camsis_m if sex==1


capture drop s_secamsis
gen s_secamsis=.
replace s_secamsis = s_se_camsis_f if sex==1
replace s_secamsis = s_se_camsis_m if sex==2

********************************************************************************
*ADDING CAMISIS FEMALE FRIEND TO UK 
...
capture drop tradsecamsis
gen tradsecamsis =secamsis
replace tradsecamsis= s_secamsis if  missing(secamsis)
replace tradsecamsis =s_secamsis if sex==2 & !missing(s_se_camsis_m )


capture drop tradukcamsis
gen tradukcamsis =ukcamsis
replace tradukcamsis= s_ukcamsis if  missing(ukcamsis)
replace tradukcamsis =s_ukcamsis if sex==2 & !missing(s_uk_camsis_m )
...
capture drop domsecamsis
gen domsecamsis =secamsis
replace domsecamsis= s_secamsis if  missing(secamsis)
replace domsecamsis =s_secamsis if spdom ==1  & !missing(s_secamsis )


capture drop domukcamsis
gen domukcamsis =ukcamsis
replace domukcamsis= s_ukcamsis if  missing(ukcamsis)
replace domukcamsis =s_ukcamsis if spdom ==1  & !missing(s_ukcamsis )
...
capture drop highsecamsis
gen highsecamsis =secamsis
replace highsecamsis= s_secamsis if  missing(secamsis)
replace highsecamsis =s_secamsis if s_secamsis > secamsis & !missing(s_secamsis )


capture drop highukcamsis
gen highukcamsis =ukcamsis
replace highukcamsis= s_ukcamsis if  missing(ukcamsis)
replace highukcamsis =s_ukcamsis if s_ukcamsis > ukcamsis & !missing(s_ukcamsis )
...
capture drop meansecamsis
gen meansecamsis =secamsis
replace meansecamsis= s_secamsis if  missing(secamsis)
replace meansecamsis = (secamsis+ s_secamsis) /2 if !missing(secamsis)& !missing(s_secamsis)

capture drop meanukcamsis
gen meanukcamsis =ukcamsis
replace meanukcamsis= s_ukcamsis if  missing(ukcamsis)
replace  meanukcamsis = (ukcamsis+ s_ukcamsis) /2 if !missing(ukcamsis)& !missing(s_ukcamsis)
...
capture drop addsecamsis
gen addsecamsis =secamsis
replace addsecamsis= s_secamsis if  missing(secamsis)
replace addsecamsis = secamsis+ s_secamsis if !missing(secamsis)& !missing(s_secamsis)

capture drop addukcamsis
gen addukcamsis =ukcamsis
replace addukcamsis = s_ukcamsis if  missing(ukcamsis)
replace addukcamsis = ukcamsis+ s_ukcamsis if !missing(ukcamsis)& !missing(s_ukcamsis)
...
egen	z2ukcamsis 		=	std(ukcamsis)
egen	z2secamsis		=	std(secamsi)
...
egen	z2tradukcamsis	=	std(tradukcamsis)
egen	z2tradsecamsis	=	std(tradsecamsis)
egen	z2tradisei  	=	std(tradisei)
egen	z2tradtrei 		=	std(tradtrei)
					
egen	z2domicam   	=	std(domicam)
egen	z2domukcamsis	=	std(domukcamsis)
egen	z2domsecamsis	=	std(domsecamsis)
egen	z2domisei   	=	std(domisei)
egen	z2domtrei  		=	std(domtrei)
					
egen	z2highicam   	=	std(highicam)
egen	z2highukcamsis	=	std(highukcamsis)
egen	z2highsecamsis	=	std(highsecamsis)
egen	z2highisei   	=	std(highisei)
egen	z2hightrei  	=	std(hightrei)
					
egen	z2meanicam   	=	std(meanicam)
egen	z2meanukcamsis	=	std(meanukcamsis)
egen	z2meansecamsis	=	std(meansecamsis)
egen	z2meanisei   	=	std(meanisei)
egen	z2meantrei  	=	std(meantrei)
					
egen	z2addicam 		=	std(addicam)
egen	z2addukcamsis	=	std(addukcamsis)
egen	z2addsecamsis	=	std(addsecamsis)
